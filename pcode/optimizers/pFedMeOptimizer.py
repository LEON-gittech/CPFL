...
class pFedMeOptimizer(Optimizer):
    ...
    def step(self, local_weight_updated, closure=None):
        ...
        for group in self.param_groups: #self.param_group对应的是 self.model，即θ
            for p, localweight in zip( group['params'], weight_update):
                p.data = p.data - group['lr'] * (p.grad.data + group['lamda'] * (p.data - localweight.data) + group['mu']*p.data) #这里更新的是θ
